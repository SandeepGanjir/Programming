   ...
   {	byte[] InA, RS, RT, RD;
	...
	public static void main(String args[]) throws IOException
	   {	Main X = new Main();	Memory M = new Memory();
		...
		int pc, mLoc, BT, Binst=0, Bmiss=0, MInst=0, L1M=0, L2M=0, tlbM=0;
		 ...
		 { SA = inF.readLine();
		   ...
		   try
			...
			{ MInst++;
			  ...
			  if(BT==5) {L2M++; tlbM++; Delay += 399;}
			  else if(BT>2) {L2M++; Delay += 199;}
		...
		double L1mr=100.0*L1M/MInst, L2mr=100.0*L2M/L1M, TlbMr=100.0*tlbM/L2M;
		
		DecimalFormat df = new DecimalFormat("##0.00#");
		System.out.println(df.format(IPC)+"     "+df.format(L1mr)+"%     "+df.format(L2mr)+"%     "+df.format(PredA)+"%     "+df.format(TlbMr)+"%");
		//
		System.out.println("\n  Instructions \t: "+NoIns+"\n  Delays \t: "+Delay+"\n  Branch Inst \t: "+Binst);
